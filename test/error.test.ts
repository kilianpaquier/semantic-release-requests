...
describe("getConfigError", () => {
    ...
    test("should have a valid candidates error", () => {
        // Arrange
        const code = "EINVALIDCANDIDATES"

        // Act
        const error = getConfigError("candidates", "some invalid candidates")

        // Assert
        expect(error.code).toEqual(code)
        expect(error.message).toContain("'candidates'")
        expect(error.details).toContain("must be a valid array of release candidates")
        expect(error.details).toContain("some invalid candidates")
    })

    test("should have a valid assets error", () => {
        // Arrange
        const code = "EINVALIDASSETS"

        // Act
        const error = getConfigError("assets", "some invalid assets")

        // Assert
        expect(error.code).toEqual(code)
        expect(error.message).toContain("'assets'")
        expect(error.details).toContain("must be a valid list of strings")
        expect(error.details).toContain("some invalid assets")
    })
